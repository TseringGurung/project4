...
*/
Novel::Novel(std::string name, std::string author, int page_count,
...
**/
void Novel::setGenre(const std::string& genre) { genre_ = genre; }
...
void Novel::calculateAverageRating() {
  ...
  avg_rating_ = runningTotal / reviews_.size();
}

/**
@post     : displays Novel data in the form:
"[title_] is written by [author_]. Genre: [genre_]. [It has/ It does not have] a
film adaptation. Page Count: [page_count_]. [It is / It is not] available
digitally.\n"
*/
void Novel::display() const {
  std::cout << getTitle() << " is written by " << getAuthor()
            << ". Genre: " << getGenre() << ". "
            << (hasFilmAdaptation() ? "It has" : "It does not have")
            << " a film adaptation. Page Count: " << getPageCount() << ". "
            << (isDigital() ? "It is" : "It is not")
            << " available digitally.\n";
}

/**
@param    : a reference to a string key to match the genre of the Novel
@post     : calls display() if the key argument is equal to genre_ (e.g.
"horror")
*/
